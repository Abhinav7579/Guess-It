...
const Game = () => {
  ...
  const[wait,iswait]=useState(true);
  ...
  if (ws.readyState === WebSocket.OPEN) {
    // socket already open → send now
    sendJoin();
  } else {
    // wait for it to open first
    ws.onopen = sendJoin;
  }
  ...
  return (
    ...
    <div className="bg-green-900 text-white h-screen  p-6" style={{ backgroundImage: "url('/bg.jpg')" }}>
      ...
      {
        start && !choosen &&
        <div className="text-white text-xl font-bold text-center mt-10 animate-bounce">
      ⚡Players joined the room
      <span className="animate-pulse text-yellow-200 ">..</span>
   
      </div>
      }
       {start && choosen && <div className="text-white text-xl font-bold text-center mt-10 animate-bounce">
        ...
        <div className="mt-[70px] flex justify-evenly ">
            ...
            <div className="w-[160px] h-[160px] lg:ml-[80px] lg:w-[300px] lg:h-[300px] ">
                ...
                <div className="mt-[80px] lg:ml-[90px] lg:mt-[120px] bg-blue-700 p-2 rounded-2xl">
                    ...
                    <button  disabled={!secret || secret < 1 || secret > 100} className="bg-green-500 font-bold  m-1 p-1 rounded-xl cursor-pointer hover:bg-red-600" onClick={()=>{isStart(true) ,setChoosen(true)}}>Start</button>
                </div>
}
